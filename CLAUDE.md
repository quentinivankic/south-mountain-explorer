...
- **Parking — the per-lot VISION verdict is the truth (decided 2026-08-12).**
  ...
  the same fact behind #54. Tools and verdicts live in `scripts/parking-adjud/`
  (graduated 2026-09-13; brief in `docs/parking-adjudication-handoff.md`).
  **The verdicts SHIP** via `public/areas/parking-verdicts.json`: generated by
  `scripts/build-parking-verdicts.py`, applied by `sweep-parking-verdicts.py`,
  honoured by `build-parking-pool.py` and `add-parking.py`, matched by footprint
  or `osm` id through `scripts/_parking_verdicts.py`. Aerial tiles and OSM
  extracts stay on the homelab under `/mnt/raid/trekdex/`.
