...
>
> Shipped lots carry no OSM id, so the verdicts are matched to them **by
> footprint**, most confident rule first: a shipped lot is a judged lot when it
> sits at the bounding-box centre of one of the verdict's rings (exactly what
> Overpass `out center` ships — this reaches the crescent and L-shaped lots
> whose centre falls up to 32 m off their own pavement), inside a ring, within
> 10 m of a ring's edge, or within 20 m of the verdict's position (the only test
> a node-only lot has). When two verdicts claim a lot the stronger rule wins,
> then the nearer, so a KEEP beside a wide DROP is never dropped by its
> neighbour. A fixed radius was rejected with numbers: covering the real
> out-center-vs-centroid gap (a Zion relation at 15 m, a North Mountain lot at
> 41 m) needed circles that would have swallowed unjudged neighbours in
> Griffith. `add-parking.py` now writes `osm` (`type/id`) on every lot it
> rolls: a JUDGED id matches exactly; an id the sidecar has not judged still
> falls back to footprint and position on purpose (a verdict is about the
> place, and OSM ids churn when a node is redrawn as an area). One shared
> matcher: `scripts/_parking_verdicts.py`. One-time side effect: the first roll
> after this reports every area with OSM parking as "updated", because the new
> key makes stored and recomputed lots compare unequal — expected, not a
> regression.
>
> **`public/areas/parking-verdicts.json` EXISTS** — generated by
> `scripts/build-parking-verdicts.py` from the four stores (292 judged lots
> after folding 4 same-cluster duplicates: 210 KEEP / 82 DROP; 254 carry a
> footprint). Three consumers: `scripts/sweep-parking-verdicts.py` applies
> DROPs to published geom, `scripts/build-parking-pool.py --verdicts` keeps
> them out of the global pool, and `add-parking.py` removes them from the
> state-wide candidates (OSM and federal) before assignment so a roll cannot
> resurrect one. The sweep and the pool builder refuse to empty an area; the
> roll deliberately does not — there the per-lot verdict is the source of
> truth, so an area whose every lot is a judged DROP clears and becomes
> eligible for federal fill (no area is in that state today: the sweep refused
> none). `scripts/test_parking_verdicts.py` covers matcher, generator and
> consumers.
>
> **Measured effect, first payload:** only **8 of the 82 DROPs** were ever on
> the map (the other 74 never passed the geometric gates), so the sweep removed
> 8 lots from 4 areas (Griffith 3, Zion NP 3, Phoenix Mtns Preserve 1, Stough
> Park 1) and the pool went 30,840 → 30,832 — every one a storage yard,
