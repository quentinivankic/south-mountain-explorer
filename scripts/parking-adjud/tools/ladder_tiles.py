...
measured cause of the #1596 class of false drops. Imagery is NAIP first with
ESRI as the fallback (ESRI alone failed 61 of 61 Z3 frames on 2026-09-13), paced
at PACE_S with a 0/6/15 s retry ladder; the frame header names the source used.

Usage: python3 ladder_tiles.py <slug> <fid,fid,...|all-served>
Writes <slug>_ladder/<fid>_z1.png,_z2.png,_z3.png (skip-if-exists)."""
import io, json, math, os, sys, glob, time, urllib.request
...
# Lesson 5 (judge_lessons.md): NAIP is the PRIMARY source and ESRI the fallback,
# paced at about one request a second with a 0 / 6 / 15 s retry ladder per source.
# ESRI throttles under burst by firing fast failures and then HANGING every
# connection, which reads as a network outage and is not one. The first Colorado
# run (2026-09-13) hit exactly that: 61 of the first 61 Z3 frames failed on ESRI
# while NAIP answered the same requests in 4 s. Returns (image, source-tag).
PACE_S=1.0
def fetch(x0,y0,x1,y1,W,H):
    for src,tmpl in (("NAIP",NAIP),("ESRI",ESRI)):
        url=tmpl.format(x0=x0,y0=y0,x1=x1,y1=y1,W=W,H=H)
        for back in (0,6,15):
            if back: time.sleep(back)
            try:
                im=Image.open(io.BytesIO(urllib.request.urlopen(urllib.request.Request(url,headers={"User-Agent":"trekdex/1.0"}),timeout=40).read())).convert("RGB")
                time.sleep(PACE_S)
                return im,src
...
def render(slug,fids):
    ...
    for fid in fids:
        ...
        for zname,half,PX in LADDER:
            # One frame per rung, NAIP first with ESRI fallback (the old ESRI-only
            # frame and its optional "_naip" variant are gone: NAIP IS the frame).
            out=f"{D}/{fid:04d}_{zname}.png"
            if os.path.exists(out): continue
            dlat=half/111320.0; dlon=half/(111320.0*math.cos(la*math.pi/180))
            x0,y0,x1,y1=lo-dlon,la-dlat,lo+dlon,la+dlat
            im,isrc=fetch(x0,y0,x1,y1,PX,PX)
            if im is None: print(f"FETCH FAIL {out}",flush=True); continue
            ...
            mpp=2*half/PX
            hdr=f"#{fid} {zname.upper()} · {int(2*half)} m across ({mpp:.2f} m/px) · {isrc} · red = mapped lot · yellow = our trails"
            dr.rectangle([0,0,PX,32],fill=(0,0,0,170)); dr.text((8,7),hdr[:118],fill=(255,235,120,255),font=Fnt)
            im.save(out); n+=1
        print(f"fid{fid} done",flush=True)
