...
import { Square, Trash2, Loader2, MapPinned, Compass } from "lucide-react";
import {
  stopRecording,
  discardRecording,
...
export function RecordingPanel({ active, trails, error, onFinish }: Props) {
  ...
  const targetTrail =
    active.mode === "trail" && active.trailId
      ? trails.find((t) => t.id === active.trailId)
      : null;
    ...
    <div className="pointer-events-auto absolute z-[1000] left-4 right-4 bottom-4 rounded-2xl bg-card/95 backdrop-blur-sm shadow-[var(--shadow-elev)] border border-border/60 p-4">
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="inline-flex items-center gap-1 text-[10px] uppercase tracking-wider text-muted-foreground">
          {active.mode === "roam" ? (
            <>
              <Compass className="size-3" /> Free roam
            </>
          ) : (
            <>
              <MapPinned className="size-3" /> On trail
            </>
          )}
        </span>
      </div>
      {targetTrail && (
        <div className="mt-1.5 text-sm font-semibold truncate">{targetTrail.name}</div>
      )}
